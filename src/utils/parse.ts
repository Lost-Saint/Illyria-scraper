 ...
 */
export const typo = ([source]: Data): TranslationInfo['typo'] => source?.[1]?.[0]?.[4] ?? undefined
...
export const pronunciation = {
	 ...
	 */
	query: ([source]: Data): TranslationInfo['pronunciation']['query'] => source?.[0] ?? undefined,
	...
	translation: ([
		,
		target
	]: Data): TranslationInfo['pronunciation']['translation'] => target?.[0]?.[0]?.[1] ?? undefined
}
...
export const list = {
	 ...
	 */
	examples: ({ 3: extra }: Data): TranslationInfo['examples'] => extra?.[2]?.[0]?.map(([, item]) => item) ?? [],
	 ...
	 */
	similar: ({ 3: extra }: Data): TranslationInfo['similar'] => extra?.[3]?.[0] ?? [],
...
const isObject = (
	value: unknown
): value is GenericObject<object | string | number> => typeof value === 'object'
